...
Get Unique Bike Ad
    ...
    ${bike_ad}         To Json              {"thumbnail": "elleven.jpg", "name": "Elleven Rocker", "brand": "Shimano", "price": "15"}
    ${bike_ad_id}      Create Bike Ad       ${bike_ad}           ${user_id}
    ${resp}            Get Bike Ad By Id    ${bike_ad_id}        ${user_id}
    Log                ${resp.text}
    Should Be Equal As Strings              ${resp.status_code}  200
    Dictionary Should Contain Value         ${resp.json()}       ${bike_ad['thumbnail']}
    Dictionary Should Contain Value         ${resp.json()}       ${bike_ad['name']}
    Dictionary Should Contain Value         ${resp.json()}       ${bike_ad['brand']}
    ${bike_price}                           Convert To Integer   ${bike_ad['price']}
    Dictionary Should Contain Value         ${resp.json()}       ${bike_price} 
